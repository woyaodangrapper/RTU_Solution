...
//var buffer = new CircularBuffer(3); // 初始容量为3

//// 写入
//buffer.Write(new byte[] { 1, 2, 3 });

//// 当前状态
//Console.WriteLine(buffer.Count); // 3
//Console.WriteLine(buffer.IsEmpty); // False
//Console.WriteLine(buffer.IsFull); // True

//// 再写入更多（会触发扩容）
//buffer.Write(new byte[] { 4, 5, 6, 7 }); // 扩容到 6

//// 读出数据
//var data = buffer.Read(4); // 读取 [4,5,6,7]

//Console.WriteLine(string.Join(",", data)); // 输出：4,5,6,7

//// 状态
//Console.WriteLine(buffer.Count); // 2 (缓存中还有 [6,7])
//Console.ReadLine();